...
// The comma before the state is optional because the agendas are inconsistent
// about it ("290 Greenwich Avenue, Greenwich CT 06830" on 5/13 and 5/26/2026),
// and requiring it sent those meetings to the "see the agenda" placeholder. The
// comma before the *town* stays required: it is what keeps a letterhead line
// like "Town Hall• 101 Field Point Road • Greenwich, CT 06830" from reading as a
// venue, since that line leaves no town between its one comma and the state.
...
// A Zoom-only meeting names Zoom where an in-person one names a building, and
// the wording varies agenda to agenda: "Virtual Meeting via Zoom", "Via Zoom",
// "Virtual (Zoom Only)". Matching on any one of those phrases left the others
// reading as 'unknown', which showed the "see the agenda" placeholder on a
// meeting whose location the agenda states plainly. So test how the location
// segment *starts* instead: a segment that opens with "virtual"/"via Zoom"/
// "Zoom"/"remote" is naming Zoom as the venue. A hybrid meeting's segment opens
// with the room ("Cone Room and via Zoom"), so it can't be swept up by this.
const VIRTUAL_START_RE = /^(?:virtual|via\s+zoom|zoom|remote)\b/i;

// The location can sit on its own line ("Via Zoom") or share the date line after
// a pipe ("| 7:30 AM ET | Virtual (Zoom Only)"), so both separators split it.
function isVirtualHeader(header) {
  return header
    .split(/\r?\n|\|/)
    .map(cleanLine)
    .some((segment) => VIRTUAL_START_RE.test(segment) && /zoom/i.test(segment));
}
...
export function parseAgendaLocation(text) {
  ...
  // Checked before the Zoom test, not after: a hybrid agenda names both the
  // Media Center and Zoom, and the physical venue is the answer in that case.
