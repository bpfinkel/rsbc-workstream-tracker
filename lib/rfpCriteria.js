...
//
// NOTE on criteria order: within a phase, a criterion's POSITION in the array —
// not its `key` — decides which ScoreN column it is written to and read back
// from in lib/sheets.js (submitScore maps index 0..5 onto Score1..Score6).
// Criteria are listed highest-weight-first for the scorers' benefit; the `key`
// values are kept renumbered to match position so key N always means column N.
// Reordering or inserting a criterion after scoring has begun would silently
// reinterpret existing rows — check the RFPScoring tab is empty first.
