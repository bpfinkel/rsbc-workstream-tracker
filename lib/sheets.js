...
// scored, plus whether Written and Interview scoring are each unlocked) and
// RFPScoring (one row per phase+firm+scorer submission — a scorer can freely
// resubmit, which upserts in place rather than creating a duplicate row).
// Criteria labels and point weights live in lib/rfpCriteria.js, not here.

const RFP_FIRMS_SHEET = 'RFPFirms';
const RFP_FIRMS_HEADERS = ['Firm', 'InterviewUnlocked', 'WrittenUnlocked'];
...
async function ensureRfpFirmsSheet(sheets) {
  ...
  if (!exists) {
    ...
    const rows = [RFP_FIRMS_HEADERS, ...RFP_FIRMS_SEED.map((firm) => [firm, 'FALSE', 'TRUE'])];
    await sheets.spreadsheets.values.update({
      spreadsheetId: SHEET_ID,
      range: `${RFP_FIRMS_SHEET}!A1:C${rows.length}`,
      valueInputOption: 'USER_ENTERED',
      requestBody: { values: rows }
    });
    return;
  }
  // Migration for sheets created before the WrittenUnlocked column existed —
  // extend the header row only, never touch existing data rows. listFirms()
