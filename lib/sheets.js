...
const EXTRA_MEMBERS = [
  { name: 'QA+M', role: 'Architect (external)', status: 'External', officer: false }
];
...
// row ends the active list. Only name/role/status/officer are exposed here — email and
// phone are intentionally left out until the app has a login to gate access to them.
export async function listMembers() {
  ...
  for (const row of rows) {
    ...
    const role = row[1] || '';
    const status = row[2] || '';
    members.push({ name, role, status, officer: status === 'Officer' });
  }
